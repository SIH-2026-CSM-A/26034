...
def arbitrate_field_declaration(
    ...
    if not primary_span.polygon or len(primary_span.polygon) < 3:
        return arbitrate_mrp(
            primary_span.text, "", span_id=primary_span.span_id, primary_reading=primary_span
        )

    pts = np.array(primary_span.polygon)

    # CRITICAL FIX: Explicitly cast the coordinates to integers so array slicing doesn't crash
