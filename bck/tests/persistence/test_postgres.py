...
DECLARED_ENUM_TYPES: dict[str, type[StrEnum]] = {
...
"""Every enum type this schema is *believed* to create, written out by hand.

Not what the drift guard drives off — :data:`ENUM_TYPES` below is derived from the metadata
for that. This list exists so that adding an enum type is a visible line in a review rather
than something that appears implicitly, and
:func:`test_the_declared_enum_types_are_the_ones_the_schema_builds` fails when the two
...
ENUM_TYPES: dict[str, type[StrEnum]] = {
    ...
    if isinstance(column.type, sqlalchemy.Enum) and column.type.enum_class is not None
}
"""Every PostgreSQL enum type this schema creates, against the Python enum behind it, read
from the metadata rather than listed.

Derived for the same reason :data:`TABLE_NAMES` is. A hand-maintained mapping means the
guard below passes over any type nobody remembered to add to it — it does not fail, it
simply never asks about it, which is the worse failure. ``enum_column`` does not hide the
type from this: ``column.type.name`` is the name ``pg_enum`` spells and
``column.type.enum_class`` is the Python enum behind it.
"""
...
def test_the_migration_downgrades_and_reapplies(alembic_config: Config, database_url: str) -> None:
    ...
    ``create_table`` creates the enum types implicitly and autogenerate writes no
    ...
    The set it checks comes from :data:`ENUM_TYPE_NAMES`, which is derived from the
    metadata, so a type added by a later revision is covered here without editing this
    test. It is the reason the count is not written down: it moved from six to eleven in
    one ticket.
    """
...
def test_the_metadata_registers_every_table() -> None:
    ...
    # The four CORE-004 tables live in ``app.core.market`` and ``app.core.complaints``,
    # which ``alembic/env.py`` reaches only because ``app/core/__init__.py`` imports them.
    # Drop that line and autogenerate sees nothing, writes an empty migration, and every
    # metadata-derived assertion in this file keeps passing over tables it never hears
    # about. This is the assertion that notices.
    assert {"vendors", "vendor_scans", "complaints", "product_reviews"} <= set(Base.metadata.tables)
...
def test_the_declared_enum_types_are_the_ones_the_schema_builds() -> None:
    """The hand-written list and the metadata agree, in both directions.

    :data:`ENUM_TYPES` is derived so the drift guard above cannot pass over a type nobody
    listed. That derivation is the safety property; this is the review property — an enum
    type added to a model shows up as a line someone had to write in
    :data:`DECLARED_ENUM_TYPES`, and this test is what makes forgetting it fail rather than
    pass quietly.

    Neither side reads the database, so this proves nothing about PostgreSQL. It is not
    meant to: :func:`test_every_enum_type_holds_exactly_its_python_members` does that, and
    does it over the derived mapping this test keeps honest.
    """
    assert ENUM_TYPES == DECLARED_ENUM_TYPES
