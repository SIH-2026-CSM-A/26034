...
from app.modules.vision.ocr import (
    ArbitrationResult, arbitrate_field_declaration, arbitrate_mrp,
    extract_mrp_quantity, extract_panel_text,
)
...
def test_extract_panel_text_mocked(mock_paddle, tmp_path):
    ...
    dummy_det.mkdir(); dummy_rec.mkdir()
    spans = extract_panel_text(np.zeros((10, 10, 3), dtype=np.uint8), str(dummy_det), str(dummy_rec))
...
def test_ocr_network_isolation(mock_paddle, monkeypatch, tmp_path):
    def mock_socket_init(*args, **kwargs): raise OSError("Network blocked")
    monkeypatch.setattr(socket, "socket", mock_socket_init)
    if hasattr(socket, "create_connection"): monkeypatch.setattr(socket, "create_connection", mock_socket_init)
    dummy_det, dummy_rec = tmp_path / "det", tmp_path / "rec"
    dummy_det.mkdir(); dummy_rec.mkdir()
    spans = extract_panel_text(np.zeros((10, 10, 3), dtype=np.uint8), str(dummy_det), str(dummy_rec))
...
def test_arbitration_disagreement_emits_review_marker():
    ...
    mock_span = ExtractedSpan(
        span_id=span_id, region_id="panel", polygon=[(0, 0), (10, 0), (10, 10), (0, 10)],
        text="150", confidence=0.99, source_provider=EvidenceProvider.PADDLEOCR,
    )
...
def test_synthetic_glyph_confusion_correction(mock_image_to_string, tmp_path):
    ...
    dummy_tess = tmp_path / "tessdata"
    dummy_tess.mkdir()
    def side_effect(image, config=""): return "10" if "tessedit_char_whitelist" in config else "1S0"
...
def test_arbitrate_field_declaration_wiring(mock_extract):
    ...
    primary_span = ExtractedSpan(
        span_id=span_id, region_id="panel", polygon=[(10, 10), (50, 10), (50, 50), (10, 50)],
        text="100", confidence=0.99, source_provider=EvidenceProvider.PADDLEOCR,
    )
    result = arbitrate_field_declaration(np.zeros((100, 100, 3), dtype=np.uint8), primary_span, "/fake/dir")
